...
public class AVLTree<E extends Comparable<E>>
    ...
    /**Constructor that takes a BınaryTree */
    public AVLTree(BinaryTree<E> item)
    {
        if(isBalanced(item))
            System.out.println("It is AVL Tree.");
        else
            System.out.println("It is not AVL Tree.");
    }

    public boolean isBalanced(BinaryTree<E> root){
        return checkBalance(root) != -1;
    }

    private int checkBalance(BinaryTree<E> node){
        if(node == null)
            return 0;
        int lbalance = checkBalance(node.getLeftSubtree());
        int rbalance = checkBalance(node.getRightSubtree());

        int balance=rbalance-lbalance;
        if(rbalance==-1 || lbalance==-1 || balance > 1 || balance<-1)
            return -1;
        else
            return (lbalance > rbalance) ? lbalance+1 : rbalance+1;
    }
    /*  ****************************************************************************************************************************** */
    /*</listing>*/
    // Data Fields
...
// Insert solution to programming exercise 1, section 2, chapter 9 here
    ...
    /**
     * This method is symmetric with rebalanceLeft
     * (all lefts are changed to rights and all rights are changed to lefts.)
     */
...
// Insert solution to programming exercise 3, section 2, chapter 9 here
    ...
    private void incrementBalance(AVLNode < E > node) {
        node.balance++;
        if (node.balance > AVLNode.BALANCED)
      /* if now right heavy, the overall height has increased*/
            increase = true;
        else
      /* if now balanced, the overall height has not increased */
            increase = false;
    }
